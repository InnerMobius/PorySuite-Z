...
from plugin_abstract.pokemon_data import ReadSourceFile, WriteSourceFile, MissingSourceError
import re
import os
import json
import logging
...
class SpeciesData(pokemon_data.SpeciesData):
    ...
    def parse_to_c_code(self):
        ...
        Reads species_info.h directly from disk, patches each species block
        with the in-memory data, and writes the result back.  Also updates
        pokedex_entries.h and pokedex_text_fr.h for category/description.
        """
...
class PokemonItems(pokemon_data.PokemonItems):
    ...
    def _set_items_header_path(self, rel_path: str):
        ...
        backup_rel = os.path.normpath(os.path.join("temp", "porysuite", "backups", rel_norm))
        if "ITEMS_H" in self.FILES:
            self.FILES["ITEMS_H"]["original"] = rel_norm
            self.FILES["ITEMS_H"]["backup"] = backup_rel
        ...
        if getattr(self, "EXTRACTOR", None) is not None:
            ...
            if isinstance(files, dict):
                files["ITEMS_H"] = {"original": rel_norm, "backup": backup_rel}
...
class PokemonMoves(pokemon_data.PokemonMoves):
    ...
    def _set_file_path(self, file_key: str, rel_path: str) -> None:
        ...
        entry["backup"] = os.path.normpath(os.path.join("temp", "porysuite", "backups", normalized))
...
class PokemonDataManager(pokemon_data.PokemonDataManager):
    ...
    def get_species_shiny_image_path(
        ...
        out_dir = os.path.join(self.project_info["dir"], "temp", "porysuite", "shiny", slug)
        try:
