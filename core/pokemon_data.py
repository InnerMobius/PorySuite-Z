...
class PokemonItems(pokemon_data.PokemonItems):
    ...
    def parse_to_c_code(self):
        ...
        # If the build regenerates this header from its .json (jsonproc /
        # AUTO_GEN_TARGETS), NEVER patch it — it's a build artifact. A
        # hand-written copy can drift from the engine's template AND its mtime
        # would make `make` skip the regen. items.json (the source of truth) is
        # written by save(); the build produces items.h from it.
        from core.make_targets import is_autogenerated_header
        if is_autogenerated_header(self.local_util.repo_root(),
                                   os.path.basename(header_rel)):
            return
        ...
        if payload is not None:
            ...
            try:
                ...
                if not os.path.isfile(file_path) or should_save:
                    if self.data:
                        # MATCH the file's existing on-disk format instead of
                        # hardcoding it (detected at load in pokemon_data_base):
                        # a file that escapes non-ASCII (upstream style) keeps
                        # escaping so there is no phantom diff after an upstream
                        # pull; a file already storing literal UTF-8 keeps the
                        # literals instead of churning every accented line on
                        # save. Indent width + trailing newline preserved too.
                        # Fixes the literal-UTF-8 churn AND keeps the original
                        # anti-phantom-diff behaviour. See BUGS.md.
