...
_PAUSE_BLOCK = (
    ...
    "    //\n"
    "    // GATE (ArePlayerFieldControlsLocked): only pause during FREE-ROAM.  When\n"
    "    // an interaction/coord/trigger script is running the field controls are\n"
    "    // locked (DoCB1_Overworld auto-calls LockPlayerFieldControls() the frame\n"
    "    // a script starts, and `releaseall`/`release` clear it).  During that\n"
    "    // window the player cannot move, so this feature is both pointless and\n"
    "    // HARMFUL: NpcTakeStep is the same step path a scripted `applymovement`\n"
    "    // uses, so pausing here stalls a cutscene NPC's scripted walk forever --\n"
    "    // `waitmovement` never returns and the game freezes (e.g. Bill's Sea\n"
    "    // Cottage teleporter, where the locked player stands on Bill's exit\n"
    "    // path).  NOTE: gPlayerAvatar.preventStep is NOT a reliable gate -- it\n"
    "    // is set by field effects (mount/dismount/surf/etc.) but NOT by\n"
    "    // lockall/lock, so an earlier attempt at this gate left cutscenes still\n"
    "    // freezing.  ArePlayerFieldControlsLocked() is the correct script-window\n"
    "    // flag.\n"
    "    {\n"
    "        bool8 paused = FALSE;\n"
