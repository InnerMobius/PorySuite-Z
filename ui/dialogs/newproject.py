...
from PyQt6.QtWidgets import QDialog, QFileDialog, QListWidgetItem, QApplication
...
class NewProject(QDialog):
    ...
    def __init__(self, parent=None, cancel_quits=False):
        ...
        self.plugins = [_core.plugin_info()]
        self._add_plugin_items()
        self.selected_plugin = None
        self.error_encountered = False

    def _add_plugin_items(self):
        ...
        for plugin in self.plugins:
            ...
            item.setData(Qt.ItemDataRole.UserRole, (plugin['identifier'], plugin['version']))
    ...
    def open_about(self):
        """No-op — plugin info dialog removed."""
        pass
    ...
    def update(self):
        ...
        if origin == self.ui.projectName or origin == self.ui.projectDir or origin == self.ui.pluginsList:
            ...
            plugin = self.plugins[self.ui.pluginsList.currentRow()-1]
            # Build a simple namespace with the properties the setup code needs
            _ns = type("_PluginInfo", (), {
                "project_base_repo": plugin["project_base_repo"],
                "project_base_branch": plugin.get("project_base_branch", "master"),
                "rom_base": {"repo": plugin["project_base_repo"]},
            })()
            self.selected_plugin = _ns
    ...
    def initialize_project(self):
        ...
        try:
            ...
            dest_source = os.path.join(self.project_dir, "source")
            if branch is None:
                run_command(["git", "clone", "--progress", "--verbose", repo, dest_source],
                            logger=self.logSignal)
            else:
                run_command(["git", "clone", "--progress", "--verbose",
                             "--branch", branch, repo, dest_source],
                            logger=self.logSignal)
            if self.error_encountered:
                return

            run_command(["git", "switch", "-c", project_name],
                        wdir=os.path.join(self.project_dir, "source"),
                        logger=self.logSignal)
            if self.error_encountered:
                return

            # Clone base game repo
            dest_base = os.path.join(self.project_dir, "base")
            ...
            from programsetup import _find_agbcc_source
            agbcc_dir = _find_agbcc_source() or os.environ.get("AGBCC_DIR", "agbcc")
            run_command(wdir=agbcc_dir, args=["./install.sh", dest_source],
                        logger=self.logSignal)
            if self.error_encountered:
                return
            run_command(wdir=agbcc_dir,
                        args=["./install.sh", dest_base], logger=self.logSignal)
            if self.error_encountered:
                return
    ...
    def save_project_info(self):
        ...
        self.project_info = {
            ...
            "plugin_identifier": self.ui.pluginsList.currentItem().data(Qt.ItemDataRole.UserRole)[0],
            "plugin_version": self.ui.pluginsList.currentItem().data(Qt.ItemDataRole.UserRole)[1],
